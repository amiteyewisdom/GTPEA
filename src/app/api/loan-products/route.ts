...
import { canManageLoanProducts, getStaffUser } from "@/lib/api/staff-auth";
...
export async function PUT(request: Request) {
  ...
  if (!canManageLoanProducts(role)) {
    return NextResponse.json({ error: "You do not have permission to manage loan products." }, { status: 403 });
  }
  ...
  if (productError || !product) {
    return NextResponse.json({ error: productError?.message || "Failed to update loan product." }, { status: 500 });
  }

  return NextResponse.json({ message: "Loan product updated successfully", product });
}
